...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.creditTransactions.count > 0) {
        ...
        NSMutableString *text = [NSMutableString stringWithFormat:@"%@ to %@", [transaction objectForKey:@"amount"], [transaction objectForKey:@"bucket"]];
        ...
        if ([creditItem objectForKey:@"referrer"] || [creditItem objectForKey:@"referree"]) {
            ...
            if ([creditItem objectForKey:@"referrer"]) {
                hasReferrer = YES;
                [text appendFormat:@"Refering user: %@", [creditItem objectForKey:@"referrer"]];
            }
            if ([creditItem objectForKey:@"referree"]) {
                if (hasReferrer) {
                    [text appendString:@" -> "];
                }
                [text appendFormat:@"User referred: %@", [creditItem objectForKey:@"referree"]];
            }
        }
